...
with Python and PyQT, and it is a powerful and flexible toolkit that will allow us to create a user-friendly interface.
...
as the bus lane, left-turn lane or right-turn lane. The simulation can start by clicking the "Start Simulation" button. The simulatuion will then run and the results will be displayed on the screen on a tabular format.
...
Throughout the whole proccess of the software, from inputing the parameters to displaying the results, the front-end will have error handling to ensure that the software is robust and reliable.
