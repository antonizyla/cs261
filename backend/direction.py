...
class Direction:
    ...
    def simulateUpdate(self, trafficLights, trafficlight_timing):
        # Dequeue cars #TODO SEEMS TO BE AN ISSUE HERE
        spaces_in_lanes = []
        ...
        while sum(spaces_in_lanes) != 0:  # Include condition for if there aren't any more cars to add
            ...
            if lane.goes_to(left_of(self.flows.get_direction_from())) and not lane.goes_to(right_of(
                ...
                if self.pools[0] > 0 and not lane.is_bus_lane:
                    lane.add_vehicle(Vehicle(self.flows.get_direction_from(), left_of(self.flows.get_direction_from()),
                                             VehicleType.CAR))
                    spaces_in_lanes[index] -= 1
                    self.pools[0] -= 1
                elif lane.goes_to(opposite_of(self.flows.get_direction_from())) and self.pools[
                    1] > 0 and not lane.is_bus_lane:
                    lane.add_vehicle(
                        Vehicle(self.flows.get_direction_from(), opposite_of(self.flows.get_direction_from()),
                                VehicleType.CAR))
                    spaces_in_lanes[index] -= 1
                    self.pools[1] -= 1
                    ...
                    spaces_in_lanes[index] -= 1
                    self.pools_bus[0] -= 1
                elif lane.goes_to(opposite_of(self.flows.get_direction_from())) and self.pools_bus[
                    1] > 0 and lane.is_bus_lane:
                    lane.add_vehicle(
                        Vehicle(self.flows.get_direction_from(), opposite_of(self.flows.get_direction_from()),
                                VehicleType.BUS))
                    spaces_in_lanes[index] -= 1
            ...
            # the lane goes to the right or ahead
                ...
                if self.pools[2] > 0 or self.pools_bus[2] > 0:
                    ...
                    lane.add_vehicle(
                        Vehicle(self.flows.get_direction_from(), right_of(self.flows.get_direction_from()), vehicle))
                    spaces_in_lanes[index] -= 1
                    ...
                    1] > 0 and not lane.is_bus_lane:  # Don't technically need the third condition since we don't have right turning bus lanes
                    lane.add_vehicle(
                        Vehicle(self.flows.get_direction_from(), opposite_of(self.flows.get_direction_from()),
                                VehicleType.CAR))
                    spaces_in_lanes[index] -= 1
            ...
            # lane goes ahead only
                ...
                if lane.goes_to(opposite_of(self.flows.get_direction_from())) and self.pools[
                    1] > 0 and not lane.is_bus_lane:
                    lane.add_vehicle(
                        Vehicle(self.flows.get_direction_from(), opposite_of(self.flows.get_direction_from()),
                                VehicleType.CAR))
