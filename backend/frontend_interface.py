...
def front_backend_join(junc_params, flowrates: list[FlowRates]):
    ...
    for d in [j.northerly_lanes, j.easterly_lanes, j.southerly_lanes, j.westerly_lanes]:
        ...
        overall_score += ((flowrates[i].get_flow_total() + k_pedestrian *  (1 if junc_params.has_pedestrian_crossing() else 0) * junc_params.get_crossing_time() * junc_params.get_crossing_rph()) / (k_wait * ((results[i][0] if results[i][0] != 0 else 0.01) + ((results[i][2] if results[i][2] != 0 else 0.01)**2 /( results[i][0] if results[i][0] != 0 else 0.01))) + (results[i][1] if results[i][1] != 0 else 0) * k_len))
        i += 1
